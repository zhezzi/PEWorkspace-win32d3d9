...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	// 如果传入的 pEvt->m_npcType 为空或者未设置，则默认将 m_npcType 设为 "civilian"
	if (pEvt->m_npcType[0] == '\0') {  // 检查 m_npcType 是否为空
		StringOps::writeToString("Civilian", m_npcType, 64);  // 默认值为 "civilian"
	}
	else {
		StringOps::writeToString(pEvt->m_npcType, m_npcType, 64);
	}
