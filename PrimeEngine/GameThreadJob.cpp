    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				// Copy plane information from camera scene node into event
				for (int i = 0; i < 6; ++i) {
					for (int j = 0; j < 4; ++j) {
						drawEvt->m_frustumPlanesMatrix[i].m[0][j] = pcam->m_frustumPlanesMatrix[i].m[0][j]; 
						//PEINFO("......Location: GameThreadJob: drawEvt->m_frustumPlanesMatrix[%f].m[0][&f]: %f\n",i,j, drawEvt->m_frustumPlanesMatrix[i].m[0][j]);

					}
				}

				drawEvt->m_cameraWorld = pcam->m_cameraWorld;
