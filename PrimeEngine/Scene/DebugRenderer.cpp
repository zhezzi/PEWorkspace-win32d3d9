...
void DebugRenderer::drawMeshBoundingVolume(Vector3 minPoint, Vector3 maxPoint, Matrix4x4 base, Vector3 color, float timeToLive)
{
	// AABB 的 8 个顶点
	Vector3 vertices[8] = {
		Vector3(minPoint.m_x, minPoint.m_y, minPoint.m_z),
		Vector3(maxPoint.m_x, minPoint.m_y, minPoint.m_z),
		Vector3(maxPoint.m_x, maxPoint.m_y, minPoint.m_z),
		Vector3(minPoint.m_x, maxPoint.m_y, minPoint.m_z),
		Vector3(minPoint.m_x, minPoint.m_y, maxPoint.m_z),
		Vector3(maxPoint.m_x, minPoint.m_y, maxPoint.m_z),
		Vector3(maxPoint.m_x, maxPoint.m_y, maxPoint.m_z),
		Vector3(minPoint.m_x, maxPoint.m_y, maxPoint.m_z)
	};

	// 绘制 AABB 的 12 条边
	drawBoundingBoxEdge(vertices[0], vertices[1], color, base);
	drawBoundingBoxEdge(vertices[1], vertices[2], color, base);
	drawBoundingBoxEdge(vertices[2], vertices[3], color, base);
	drawBoundingBoxEdge(vertices[3], vertices[0], color, base);

	drawBoundingBoxEdge(vertices[4], vertices[5], color, base);
	drawBoundingBoxEdge(vertices[5], vertices[6], color, base);
	drawBoundingBoxEdge(vertices[6], vertices[7], color, base);
	drawBoundingBoxEdge(vertices[7], vertices[4], color, base);

	drawBoundingBoxEdge(vertices[0], vertices[4], color, base);
	drawBoundingBoxEdge(vertices[1], vertices[5], color, base);
	drawBoundingBoxEdge(vertices[2], vertices[6], color, base);
	drawBoundingBoxEdge(vertices[3], vertices[7], color, base);
}

void DebugRenderer::drawBoundingBoxEdge(Vector3 startPoint, Vector3 endPoint, Vector3 color, Matrix4x4 base)
{

	float linePoints[] = {
		// 起始点位置
		startPoint.m_x, startPoint.m_y, startPoint.m_z,
		// 起始点颜色
		color.m_x, color.m_y, color.m_z,
		// 终止点位置
		endPoint.m_x, endPoint.m_y, endPoint.m_z,
		// 终止点颜色
		color.m_x, color.m_y, color.m_z
	};

	// 使用 DebugRenderer 的 createLineMesh 函数绘制线
	DebugRenderer::Instance()->createLineMesh(false, base, linePoints, 2, 0);
}
