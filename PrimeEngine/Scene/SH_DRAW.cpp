...
// SingleHandler_DRAW::do_GATHER_DRAWCALLS() is an event handler 
// where mesh instance is potentially added to draw list that subsequently results in submission to graphics pipeline.
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
   ...
   /* if (true && pMeshCaller->m_performBoundingVolumeCulling)
	...
	if (pMeshCaller->m_performBoundingVolumeCulling)
	{
		...
		// Interate mesh instances
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			MeshInstance* pInst = pMeshCaller->m_instances[iInst].getObject<MeshInstance>();
			PEINFO("...Location: do_GATHER_DRAWCALLS: aabb..%f\n", pMeshCaller->aabbVertices);
			...
			{
				Handle hSkeleton = pInst->getFirstParentByTypePtr<SkeletonInstance>()->getFirstParentByType<SceneNode>();
				// allow skeleton to be in chain
				
				if (hSkeleton.isValid())
				{
					hParentSN = hSkeleton.getObject<SkeletonInstance>()->getFirstParentByType<SceneNode>();
				}
			...
			// Store aabbVertices from mesh.h to meshinstance.h
			for (int i = 0; i < 8; ++i) {
				aabbVertices[i] = worldMatrix * pMeshCaller->aabbVertices[i];
			}

			//Matrix4x4 worldMatrixforFrustum = pDrawEvent->m_cameraWorld;
			

			// If aabb inside the frustum and do boundingvolume culling 
			bool isInsideFrustum = true;
			for (int i = 0; i < 6; ++i) // Iterate 6 plane equations
			{
				if (!pDrawEvent) {
				...
				//Matrix4x4 inverseTransposeWorldMatrix = worldMatrixforFrustum.inverse().transpose();
				// Store frustumplane information from draw event
				Vector4 frustumPlane = Vector4 (pDrawEvent->m_frustumPlanesMatrix[i].m[0][0], // A
												pDrawEvent->m_frustumPlanesMatrix[i].m[0][1], // B
												pDrawEvent->m_frustumPlanesMatrix[i].m[0][2], // C
												pDrawEvent->m_frustumPlanesMatrix[i].m[0][3]);// D

				int outsideVertexCount = 0;
				for (int j = 0; j < 8; ++j) // 8 vertices of AABB
				{
					Vector4 vertex = Vector4(aabbVertices[j].m_x, aabbVertices[j].m_y, aabbVertices[j].m_z, 1.0f);

					// Calculate distance
					float distance = frustumPlane.m_x * vertex.m_x +
						frustumPlane.m_y * vertex.m_y +
						frustumPlane.m_z * vertex.m_z +
						frustumPlane.m_w * vertex.m_w;
					if (distance < 0)
					{
						outsideVertexCount++; // Count number of vertices outside the frustum
					}
				}
				if (outsideVertexCount == 8) // If all vertices outside the frustum
				{
			...
			// If all vertices inside the frustum
			if (isInsideFrustum)
			{
				pInst->m_culledOut = false;
				++pMeshCaller->m_numVisibleInstances;
			...
			// Get other parameters which debugrender needs
			Vector3 minPoint, maxPoint;
			minPoint = Vector3(FLT_MAX, FLT_MAX, FLT_MAX);
			maxPoint = Vector3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
			for (int i = 0; i < 8; ++i) 
			{
				Vector3 vertex = aabbVertices[i]; 

				// Update min
				if (vertex.m_x < minPoint.m_x) minPoint.m_x = vertex.m_x;
				if (vertex.m_y < minPoint.m_y) minPoint.m_y = vertex.m_y;
				if (vertex.m_z < minPoint.m_z) minPoint.m_z = vertex.m_z;

				// Update max
				if (vertex.m_x > maxPoint.m_x) maxPoint.m_x = vertex.m_x;
				if (vertex.m_y > maxPoint.m_y) maxPoint.m_y = vertex.m_y;
				if (vertex.m_z > maxPoint.m_z) maxPoint.m_z = vertex.m_z;
			}
			...
			// Draw aabb
			DebugRenderer::Instance()->drawMeshBoundingVolume(minPoint, maxPoint, worldMatrix, color, timeToLive);

			
			// 近裁剪平面顶点
			Vector3 nearTopLeft = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[0], pDrawEvent->m_frustumPlanesMatrix[3], pDrawEvent->m_frustumPlanesMatrix[4]);
			Vector3 nearTopRight = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[1], pDrawEvent->m_frustumPlanesMatrix[3], pDrawEvent->m_frustumPlanesMatrix[4]);
			Vector3 nearBottomLeft = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[0], pDrawEvent->m_frustumPlanesMatrix[2], pDrawEvent->m_frustumPlanesMatrix[4]);
			Vector3 nearBottomRight = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[1], pDrawEvent->m_frustumPlanesMatrix[2], pDrawEvent->m_frustumPlanesMatrix[4]);

			// 远裁剪平面顶点
			Vector3 farTopLeft = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[0], pDrawEvent->m_frustumPlanesMatrix[3], pDrawEvent->m_frustumPlanesMatrix[5]);
			Vector3 farTopRight = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[1], pDrawEvent->m_frustumPlanesMatrix[3], pDrawEvent->m_frustumPlanesMatrix[5]);
			Vector3 farBottomLeft = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[0], pDrawEvent->m_frustumPlanesMatrix[2], pDrawEvent->m_frustumPlanesMatrix[5]);
			Vector3 farBottomRight = calculateIntersectionPoint(pDrawEvent->m_frustumPlanesMatrix[1], pDrawEvent->m_frustumPlanesMatrix[2], pDrawEvent->m_frustumPlanesMatrix[5]);

			Vector3 frustumpoint[8];
			frustumpoint[0] = nearTopLeft;
			frustumpoint[1] = nearTopRight;
			frustumpoint[2] = nearBottomLeft;
			frustumpoint[3] = nearBottomRight;
			frustumpoint[4] = farTopLeft;
			frustumpoint[5] = farTopRight;
			frustumpoint[6] = farBottomLeft;
			frustumpoint[7] = farBottomRight;

			Vector3 frustumVertex[8];
			
			for (int i = 0; i < 8; ++i) // 假设 numVertices 是这个实例中的顶点数量
			{
				 frustumVertex[i] =  frustumpoint[i]; // 假设 vertices 是存储顶点的数组
			}


			Vector3 colorFrustum(1.0f, 1.0f, 0.0f); // Yellow
			
			Matrix4x4 worldMatrixFrustum;
			worldMatrixFrustum.loadIdentity();
			/*
			// Draw frustum 
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[0], frustumpoint[1], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[2], frustumpoint[3], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[4], frustumpoint[5], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[6], frustumpoint[7], colorFrustum, worldMatrixFrustum);

			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[0], frustumpoint[4], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[1], frustumpoint[5], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[2], frustumpoint[6], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[3], frustumpoint[7], colorFrustum, worldMatrixFrustum);

			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[0], frustumpoint[2], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[1], frustumpoint[3], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[4], frustumpoint[6], colorFrustum, worldMatrixFrustum);
			DebugRenderer::Instance()->drawBoundingBoxEdge(frustumpoint[5], frustumpoint[7], colorFrustum, worldMatrixFrustum);
			*/
		}
	}
...
Vector3 SingleHandler_DRAW::calculateIntersectionPoint(const Matrix4x4& p1, const Matrix4x4& p2, const Matrix4x4& p3)
{
	float A1 = p1.m[0][0], B1 = p1.m[0][1], C1 = p1.m[0][2], D1 = p1.m[0][3];
	float A2 = p2.m[0][0], B2 = p2.m[0][1], C2 = p2.m[0][2], D2 = p2.m[0][3];
	float A3 = p3.m[0][0], B3 = p3.m[0][1], C3 = p3.m[0][2], D3 = p3.m[0][3];

	// 使用克莱姆法则求解线性方程组
	float denom = A1 * (B2 * C3 - B3 * C2) - B1 * (A2 * C3 - A3 * C2) + C1 * (A2 * B3 - A3 * B2);

	float x = (-D1 * (B2 * C3 - B3 * C2) + B1 * (-D2 * C3 + C2 * D3) - C1 * (-D2 * B3 + B2 * D3)) / denom;
	float y = (-A1 * (-D2 * C3 + C2 * D3) + D1 * (A2 * C3 - A3 * C2) - C1 * (A2 * D3 - A3 * D2)) / denom;
	float z = (-A1 * (B2 * D3 - B3 * D2) + B1 * (A2 * D3 - A3 * D2) + D1 * (A2 * B3 - A3 * B2)) / denom;
