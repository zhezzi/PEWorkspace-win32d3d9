...
Handle PositionBufferCPUManager::ReadVertexBuffer(const char *filename, const char *package, const char *tag)
{
	...
	min_vertex = pvbcpu->min_vertex;
	max_vertex = pvbcpu->max_vertex;


	// 打印出来检查是否成功赋值
	PEINFO("......Location:PositionBufferCPUManager: Min X: %f, Min Y: %f, Min Z: %f\n", min_vertex.m_x, min_vertex.m_y, min_vertex.m_z);
	PEINFO("......Location:PositionBufferCPUManager: Max X: %f, Max Y: %f, Max Z: %f\n", max_vertex.m_x, max_vertex.m_y, max_vertex.m_z);
